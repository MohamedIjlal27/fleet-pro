...
const ImportAssetModal: React.FC<ImportAssetModalProps> = ({
  open,
  onClose,
}) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      for (const asset of csvData) {
        try {
          const createdAsset = await createVehicle({
            make: asset["Manufacturer"] || "",
            model: asset["Model"] || "",
            trim: "",
            vin: asset["Serial Number"] || null,
            chasisNo: asset["Asset ID"] || null,
            plateNumber: asset["Asset Name"] || "",
            ...
            transmissionType: "",
            status: asset["Status"],
            garageId: Number(asset["Location"]),
            category: asset["Asset Type"] || null,
            otherCategory: asset["Other Asset Type"] || null,
            isAsset: true, // Flag to identify as an asset
            metadata: { description: asset["Description"] || "" },
          });

          toast.success(`Asset for ${asset["Asset ID"]} created successfully.`);
        } catch (error: any) {
          toast.error(
            `Failed to create asset for ${asset["Asset ID"]}: ${error.message}`
          );
        }
      }
    ...
    >
      ...
      <DialogContent>
        <Box>
          ...
          >
            ...
            <li>
              Make sure that the first row of your CSV file contains column
              headers.
            </li>
            <li>Asset Type: {Object.keys(assetTypeOptions).join(", ")}</li>
            <li>
              Location:{" "}
              {garages
                .map((garage) => `${garage.id}:${garage.name}`)
                .join(", ")}
            </li>
            <li>
              Status:{" "}
          ...
          <Link
            target="_blank"
            href="/download/csv/tpl-import-assets.csv"
            sx={{ marginRight: 2, color: "#0070f3" }}
          >
            tpl-import-assets.csv
          </Link>
        </Box>
      </DialogContent>
      <DialogActions sx={{ bgcolor: "", pt: 2 }}>
        <Button
          onClick={onClose}
          variant="contained"
          sx={{ mr: 1, backgroundColor: "#6C757D" }}
        >
          Close
        </Button>
        <Button
          ...
          variant="contained"
          sx={{ mr: 1, backgroundColor: "#727cf5" }}
        >
          Submit
        </Button>
      </DialogActions>

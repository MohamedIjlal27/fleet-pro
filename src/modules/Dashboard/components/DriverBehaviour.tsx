...
const DriverBehaviour: React.FC<DriverBehaviourProps> = ({ filters }) => {
  // State for API data
  ...
  const fetchDriverData = useCallback(async () => {
    try {
      ...
      const apiParams: DashboardFilterParams = {
        ...(filters || {}),
        wg7: true, // Use wg7 for bad drivers chart only
      };
      ...
      console.log('Driver Behaviour using all filters:', apiParams);
      ...
      try {
        if (responseData && Array.isArray(responseData) && responseData.length > 0) {
          // Find the object containing wgBadDriversChart
          const badDriversObj = responseData.find(item => item && typeof item === 'object' && 'wgBadDriversChart' in item);
          const widgetData = badDriversObj?.wgBadDriversChart;

          console.log('wgBadDriversChart data:', widgetData);
          ...
          if (widgetData && typeof widgetData === 'object' &&
            ...
            const enhancedDrivers = widgetData.top_drivers.map((driver: Driver, index: number) => {
              ...
              return {
                ...
                image: `/src/assets/admin/default-avatar-150x150.jpg`,
              };
            }).filter(Boolean); // Remove any null entries

            if (enhancedDrivers.length > 0) {
              setDrivers(enhancedDrivers);
              console.log('Driver Behavior data loaded:', enhancedDrivers);
              console.log('Total drivers found:', enhancedDrivers.length);
            } else {
              console.warn('Driver Behavior: No valid drivers in the data');
              setDrivers([]);
            }
          } else {
            console.warn('Driver Behavior: No valid top_drivers array found');
            setDrivers([]);
          }
        } else {
          console.warn('Driver Behavior: Invalid API response format');
          setDrivers([]);
        }
      } catch (parseError) {
      ...
      console.error('Error fetching driver behavior data:', err);
    ...
    datasets: [
      {
        ...
        backgroundColor: 'rgba(59, 130, 246, 0.2)', // blue-500 w/ opacity
        borderColor: '#3B82F6',
        borderWidth: 2,
          ...
          <>
            ...
            {drivers.length > 0 && (
              <Box sx={{ display: 'flex', justifyContent: 'center', mt: 2, mb: 2 }}>
                <Box className="bg-blue-50 rounded-lg p-3 text-center" sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <Typography sx={{ color: '#3B82F6', fontWeight: 'bold', fontSize: '28px', lineHeight: 1 }}>
                    {drivers[selectedDriverIndex]?.overall_score || 0}%
                  </Typography>
                  <Typography sx={{ color: '#3B82F6', fontSize: '13px' }}>
                    overall score
                  </Typography>
                </Box>
              </Box>
            ...
            {drivers.length > 1 && (
              <Box className="absolute top-2 right-2 flex gap-1">
                ...
                <IconButton size="small" sx={{ bgcolor: '#3B82F6', color: 'white', width: 30, height: 30 }} onClick={handleNextDriver}>
            ...
            {drivers.map((driver, index) => (
              <Box
                ...
                sx={{
                  ...
                  alignItems: 'center',
                  gap: 1.5,
                  bgcolor: index === selectedDriverIndex ? 'rgba(59, 130, 246, 0.1)' : 'transparent',
                  borderRadius: '8px',
                  p: 0.7,
                  minHeight: '48px',
                  cursor: 'pointer'

...
import { fetchNewAlerts, readAlert } from '@/modules/core/apis/apis';
...
export default function NotificationDropdown() {
  ...
  const loadAlerts = async () => {
    try {
      const response = await fetchNewAlerts();
      //console.log('Alerts response:', response);

      // Check for unread notifications
      const unreadNotifications = response.filter((notification: INotification) => !notification.read);

      // Update unread count
      setUnreadCount(unreadNotifications.length);
      setNotifying(unreadNotifications.length > 0); // Set notifying state based on unread notifications
  ...
  const handleClick = async () => {
    ...
    // Only mark notifications as read and update notifying state if there are unread notifications
    ...
    if (unreadNotifications.length > 0) {
      ...
      try {
        for (const notification of unreadNotifications) {
          await readAlert(notification.id, true);
        }

        // Set notifying to false after reading notifications
        setNotifying(false);
